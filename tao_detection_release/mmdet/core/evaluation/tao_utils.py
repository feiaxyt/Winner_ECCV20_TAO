...
def tao_eval(result_files, result_types, lvis, ann_file, max_dets=(100, 300, 1000), existing_json=None):
    ...
    if result_types == ['proposal_fast']:
        ar = tao_fast_eval_recall(result_files, lvis, np.array(max_dets))
        ...
        for cat_id in range(1, 1231):
            ar = tao_fast_eval_recall(result_files, lvis, np.array(max_dets), category_id=cat_id)
            for i, num in enumerate(max_dets):
    ...
    for res_type in result_types:
        ...
        lvisEval = TAOEval(ANNOTATION_PATH, result_file, iou_type)
...
def tao_fast_eval_recall(results,
                     lvis,
                     max_dets,
                     category_id=None,
